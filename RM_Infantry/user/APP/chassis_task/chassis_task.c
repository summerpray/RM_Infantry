...
/*----------------------------扭屁股模式----------------------------	*/	
		case CHASSIS_SHAKE:  
			...
			if(IF_KEY_PRESSED_F && Chass_Switch_F == 1)
			{
				Chass_Switch_F = 0;
				Chass_Key_F_Change ++;
				Chass_Key_F_Change %= 2;   //按奇数次有效，偶数次无效，按一次开再按一次关
			}
			//可以前后移动,按下左右平移、QE、切换机械模式退出
			if(Chass_Key_F_Change)
			{
				Chassis_Mode = CHASSIS_GYRO_MODE;//陀螺仪模式,底盘跟随云台动
				
				Chassis_Keyboard_Move_Calculate(STANDARD_MAX_NORMAL, TIME_INC_NORMAL);
				CHASSIS_CORGI_Mode_Ctrl( REVOLVE_MAX_CORGI, REVOLVE_SLOPE_CORGI);              //扭屁股位置不变版本
//			CHASSIS_CORGI_Mode_Ctrl_Time( REVOLVE_MAX_CORGI, REVOLVE_SLOPE_CORGI);		     //扭屁股时间不变版本		
			}
			else
			{
				Chassis_Action = CHASSIS_NORMAL; //退出扭屁股模式，进入普通模式
			}
		break;
/*----------------------------小陀螺模式---------------------------	*/			
       ...
       if(!IF_KEY_PRESSED_R)
			...
			{
				Chassis_Action = CHASSIS_GYRO_MODE; //退出小陀螺模式，进入普通模式
			}
		break;
/*----------------------------爬坡模式---------------------------	*/			
		case CHASSIS_UP:      
...
/*----------------------------------------------****R按一下则进入小陀螺模式****----------------------------------------------------*/	
	...
	else if (IF_KEY_PRESSED_R && !IF_KEY_PRESSED_CTRL  && Chass_Switch_R == 1)
	{
		Chass_Switch_R = 0;
		Chass_Key_R_Change ++;
	...
	if (ulCurrentTime >= ulDelay)//每10ms变化一次斜坡量
	{
		...
		if(Chassis_Action == CHASSIS_NORMAL && !KEY_PRESSED_OFFSET_SHIFT)//只有一般模式下才判断速度突变情况,防止打滑
		{
			...
			Slope_Chassis_Move_Fron = (int16_t)( Chassis_Standard_Move_Max * 
					Chassis_Key_MoveRamp( IF_KEY_PRESSED_S, &timeXFron, timeInc_Saltation, TIME_DEC_NORMAL ) );

			Slope_Chassis_Move_Back = (int16_t)( -Chassis_Standard_Move_Max * 
					Chassis_Key_MoveRamp( IF_KEY_PRESSED_W, &timeXBack, timeInc_Saltation, TIME_DEC_NORMAL ) );
		...
		else		//其他模式不需要进行速度方向突变特殊处理
		{
			...
			Slope_Chassis_Move_Fron = (int16_t)( Chassis_Standard_Move_Max * 
					Chassis_Key_MoveRamp( IF_KEY_PRESSED_S, &timeXFron, timeInc, TIME_DEC_NORMAL ) );

			Slope_Chassis_Move_Back = (int16_t)( -Chassis_Standard_Move_Max * 
					Chassis_Key_MoveRamp( IF_KEY_PRESSED_W, &timeXBack, timeInc, TIME_DEC_NORMAL ) );
