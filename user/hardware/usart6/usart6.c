...
void usart6_init(u32 bound)
{
	...
	//USART6端口配置
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9; //GPIOG9 RX     DMA通道5数据流1
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用功能
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;	//速度50MHz
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL; //不上拉
	GPIO_Init(GPIOG,&GPIO_InitStructure); //初始化PG9
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14; //GPIOG14 TX   DMA通道5数据流7
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用功能
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;	//速度50MHz
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL; //不上拉
